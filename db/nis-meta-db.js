...
export default class NisDBHandler {
    ...
    static addNewFolder(username, syncPath) {
        ...
        _.each(fileList, (file) => {
            databases.nisMetadataDb.insert(metaUtils.getFileMetadata(username, file));
        });
    }
    ...
    static addNewFile(username, fullPath) {
        ...
        databases.nisMetadataDb.update({path: entry.path}, entry, {upsert: true}, (err, doc) => {
            if (err) {
    ...
    static insertEntry(entry) {
        databases.nisMetadataDb.insert(entry, (err) => {
        });
    }

    static updateEntry(userName, relativePath, updateEntry) {
        databases.nisMetadataDb.update({
            user: userName,
            path: relativePath
    ...
    static deleteEntryById(_id) {
        databases.nisMetadataDb.remove({_id: _id}, {}, (err, numRemoved) => {
        });
    }
    ...
    static readEntry(fullPath) {
        ...
        return new Promise((resolve) => {
            databases.nisMetadataDb.findOne({path: path}, (err, doc) => {
                if (err) {
    ...
    static updateCurrentCheckSum(fullPath, checkSum) {
        ...
        databases.nisMetadataDb.update({path: path}, {$set: {current_cs: checkSum}}, {}, (err, numReplaced) => {
        });
    }
    ...
    static updateMetadataForRenaming(oldPath, newPath) {
        ...
        databases.nisMetadataDb.find({path: {$regex: regex}}, (err, docs) => {
            _.each(docs, (doc) => {
                ...
                databases.nisMetadataDb.update({path: doc.oldPath}, doc, {}, (err, numReplaced) => {
                });
            });
        });
    }
    ...
    static removeFilesOfDeletedDirectory(fullPath) {
        ...
        databases.nisMetadataDb.find({path: {$regex: regex}}, (err, docs) => {
            _.each(docs, (doc) => {
                databases.nisMetadataDb.remove(doc, (err, numDeleted) => {
                    if (err) {
                        console.log(err);
                    }
                });
    ...
    static deleteEntry(path) {
        databases.nisMetadataDb.remove({path: path}, (err, numDeleted) => {
            if (err) {
                console.log(err);
            }
        });
    ...
    static deleteEntryBySequenceId(sequenceID) {
        databases.nisMetadataDb.remove({sequence_id: sequenceID}, (err, numDeleted) => {
        });
    }
    ...
    static getChangesOfUser(username) {
        ...
        return new Promise((resolve) => {
            databases.nisMetadataDb.find({user: username}).sort({sequence_id: 1}).exec((err, docs) => {
                if (err) {
    ...
    static getNextSequenceID() {
        ...
        return new Promise((resolve) => {
            databases.nisMetadataDb.find({}).sort({sequence_id: -1}).limit(1).exec((err, docs) => {
                if (err) {
    ...
    static removeFilesFromSync(username, syncPath) {
        ...
        _.each(fileList, (file) => {
            ...
            databases.nisMetadataDb.remove({path: tempPath}, {}, () => {
            });
        });
    }
