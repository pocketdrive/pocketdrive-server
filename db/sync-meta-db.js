...
export default class MetadataDBHandler {
    ...
    static addNewFolder(username, syncPath) {
        ...
        _.each(fileList, (file) => {
            databases.syncMetaDb.insert(metaUtils.getFileMetadata(username, file));
        });
    }
    ...
    static addNewFile(username, fullPath) {
        ...
        databases.syncMetaDb.update({path: entry.path}, entry, {upsert: true}, (err, doc) => {
            if (err) {
    ...
    static insertEntry(entry) {
        databases.syncMetaDb.insert(entry, (err) => {
        });
    }

    static updateEntry(userName, relativePath, updateEntry) {
        databases.syncMetaDb.update({user: userName, path: relativePath}, updateEntry, {upsert: true}, (err, numReplaced) => {
        });
    }
    ...
    static readEntry(fullPath) {
        ...
        return new Promise((resolve) => {
            databases.syncMetaDb.findOne({path: path}, (err, doc) => {
                if (err) {
    ...
    static updateCurrentCheckSum(fullPath, checkSum) {
        ...
        databases.syncMetaDb.update({path: path}, {$set: {current_cs: checkSum}}, {}, (err, numReplaced) => {
        });
    }
    ...
    static updateMetadataForRenaming(oldPath, newPath) {
        ...
        databases.syncMetaDb.find({path: {$regex: regex}}, (err, docs) => {
            _.each(docs, (doc) => {
                ...
                databases.syncMetaDb.update({path: doc.oldPath}, doc, {}, (err, numReplaced) => {
                });
            });
        });
    }
    ...
    static removeFilesOfDeletedDirectory(fullPath) {
        ...
        databases.syncMetaDb.find({path: {$regex: regex}}, (err, docs) => {
            _.each(docs, (doc) => {
                databases.syncMetaDb.remove(doc, (err, numDeleted) => {
                    if (err) {
                        console.log(err);
                    }
                });
    ...
    static deleteEntry(path) {
        databases.syncMetaDb.remove({path: path}, (err, numDeleted) => {
            if (err) {
                console.log(err);
            }
        });
    ...
    static deleteEntryBySequenceId(sequenceID) {
        databases.syncMetaDb.remove({sequence_id: sequenceID}, (err, numDeleted) => {
        });
    }
    ...
    static getChangesOfUser(username) {
        ...
        return new Promise((resolve) => {
            databases.syncMetaDb.find({user: username}).sort({sequence_id: 1}).exec((err, docs) => {
                if (err) {
    ...
    static getNextSequenceID() {
        ...
        return new Promise((resolve) => {
            databases.syncMetaDb.find({}).sort({sequence_id: -1}).limit(1).exec((err, docs) => {
                if (err) {
    ...
    static removeFilesFromSync(username, syncPath) {
        ...
        _.each(fileList, (file) => {
            ...
            databases.syncMetaDb.remove({path: tempPath}, {}, () => {
            });
        });
    }
